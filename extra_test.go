...
func TestEndsWithNonSpaceCharacters(t *testing.T) {
	...
	source := []byte("```\na\n```")
	var b bytes.Buffer
	err := r.Render(&b, source, p.Parse(text.NewReader(source)))
	if err != nil {
		t.Error(err.Error())
	}
...
func TestWindowsNewLine(t *testing.T) {
	...
	source = []byte("a\\\r\nb\r\n")
	var b2 bytes.Buffer
	err = r.Render(&b2, source, p.Parse(text.NewReader(source)))
	if err != nil {
		t.Error(err.Error())
	}
	if b2.String() != "<p>a<br />\nb</p>\n" {
...
func TestDeepNestedLabelPerformance(t *testing.T) {
	...
	var b bytes.Buffer
	_ = r.Render(&b, source, p.Parse(text.NewReader(source)))
	finished := nowMillis()
